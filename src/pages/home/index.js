...
import { Input, Image, Space, Swiper, FloatingBubble, Toast } from 'antd-mobile';
import {
    HomeBox,
    HomeInnerBoxFirst,
    HomeContentBox,
...
import { handleDonate, getDailyReward, handleLotteryNormal, handleLotterySuper } from "@/api";
...
const HomePage = () => {
    ...
    useEffect(() => {
        getHomeData().then(res=>{
            setHomeData(res);
            setCions({ cion: res.user_stat.total, got: false });
        });
    }, [])
    let getNum = (type) => {
        ...
        handleLotteryNormal().then(res=>{
            let timer2 = setTimeout(() => {
                ...
                store.dispatch(counterSlice.actions.update({coin:res.coin,award:res.award}))
                // clearInterval(timer);
                clearTimeout(timer2);
            }, 1200);
                    ...
                    <HomeContentBox >
                        ...
                        <GetCionsBox got={cions.got}>
                            <div className='leftBox'>
                                <div className='top'>
                                    ...
                                    <span className='topRight'>+{homeData.user_stat.daily} every day</span>
                        ...
                        <DonationBox>
                            ...
                            <DonationContent>
                                ...
                                />
                                <Button className="donateBtn" content="Donate" radius="10" background="#1AFFB2" height={44} width={86} onClick={()=>handleDonate(donateValue)}/>
                            </DonationContent>
                        </DonationBox>
                        <PioneerBox>
                            ...
                            <PioneerContent>
                                <PioneerCard bgImg={cactus}>
                                    ...
                                    <span className='des'>Use 100u ,you will have the opportunity to get 3-100u.</span>
                        ...
                        <ForestBox>
                            <Title img={forest} name="All forests" weight="bold" right={<><TeamOutline /><span>{
                            ...
                            <ForestContent>
                                ...
                                >
                                    {homeData.forest.map((item, index) => (
                                        <Swiper.Item key={index}>
                                            <SwiperItem bgImg={item.cover} active={swiperIndex == index && 'active'} onClick={() => { ref.current.swipeNext(); console.log(ref.current) }}>
                                                <span>{item.name}</span>
                                                <div><TeamOutline /><span>{item.people}</span></div>
                                            </SwiperItem>
                                        </Swiper.Item>
                                    ))}
                                </Swiper>
                                <ForestItemDes>
                                    {homeData.forest[swiperIndex].desc}
                                </ForestItemDes>
                            </ForestContent>
                        </ForestBox>
                    </HomeContentBox>
